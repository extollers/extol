...
command(extol2prolog, [In, Out]) :-
    read_file(In, BytesIn), !,
    pl_top_level(Decls, BytesIn, []), !,
    pl_write_top_level(Decls, BytesGen, []), !,
    append("% Generated by extol2prolog\n", BytesGen, BytesOut),
...
pl_write_term(X) -->
    { open_output_codes_stream(Stream),
      write_term(Stream, X, [quoted(true), namevars(true)]),
