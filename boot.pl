...
:-initialization(main).
main:-write('main'),nl,current_prolog_flag(argv,[_,Command|Args]),command(Command,Args).
...
command(extoltoprolog,[In,Out]):-write('extoltoprolog'(In, Out)),nl,read_file(In,BytesIn),!,pl_top_level(Decls,BytesIn,[]),!,pl_write_top_level(Decls,BytesGen,[]),!,append([37,32,71,101,110,101,114,97,116,101,100,32,98,121,32,101,120,116,111,108,0't,0'o,112,114,111,108,111,103,10],BytesGen,BytesOut),write_file(Out,BytesOut).
