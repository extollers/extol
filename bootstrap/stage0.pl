...
:-(xtl_to_pl_dcg_call(A,B,define(C,D,E),F,G,H,I),','(member(inline,D),','(!,;(','(member(dcg,D),','(!,','(maplist(copy_term,E,J),','(maplist(xtl_inline_dcg_clause(F),J,K,L),','(maplist(xtl_to_pl_dcg(A,G,H),L,M),','(maplist(pl_comma,K,M,N),semicolon_list(I,N))))))),','(','('.'(H,[],O),','('.'(G,O,P),append(F,P,Q))),xtl_to_pl_goal_call(A,B,define(C,D,E),Q,I)))))).
...
:-(xtl_annot_extend_params(A,dcg,D),','('.'(E,[],F),','('.'(G,F,H),append(A,H,D)))).
...
:-(xtlm_declaration_key(define(A,B,C),D),','(!,','(=(C,'.'(:(E,F),G)),;(','(member(dcg,B),','(!,','(comma_list(E,H),','(length(H,I),','(is(J,+(2,I)),=(D,/(/(A,J),'.'(dcg,[])))))))),;(','(member(predicate,B),','(!,','(comma_list(E,H),','(length(H,J),=(D,/(/(A,J),'.'(pred,[]))))))),','(!,','(comma_list(E,H),','(length(H,I),','(is(J,+(1,I)),=(D,/(/(A,J),'.'(fun,[])))))))))))).
...
:-('__contract_free_xtl_eval_call'(A,define(B,C,D),E),','(copy_term(D,F),;(','(member(dcg,C),','(!,orlist(xtl_eval_call_dcg(A,E),F))),;(','(member(predicate,C),','(!,orlist(xtl_eval_call_pred(A,E),F))),orlist(xtl_eval_call_fun(A,E),F))))).
